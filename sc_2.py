...
Original file is located at
    https://colab.research.google.com/drive/1NGkVchDQK9sepxLwGRGGFz_ZiNECn-jm
"""
...
class SimpleRadarCounter:
    ...
    def __init__(self, config):
        ...
        # Novos atributos para tracking por ID e intervalo de registro
        self.active_people_ids = {}  # id: timestamp_ultimo_visto
        self.last_record_time = 0
        self.timeout_saida = 3  # segundos para considerar saída
        ...
        while self.is_running:
            try:
                ...
                if self.serial_connection.in_waiting > 0:
                    ...
                    if falha_leitura_contador >= 100:  # Ex: 100 ciclos * 0.1s = 10 segundos sem dados
    ...
    def get_person_id(self, person):
        # Se já vier um id, use ele
        if "id" in person:
            return str(person["id"])
        # Senão, gera um id aproximado pela posição (x, y arredondados)
        x = round(person.get("x", 0), 1)
        y = round(person.get("y", 0), 1)
        return f"{x}_{y}"

    def update_people_tracking(self, active_people):
        """Tracking robusto por ID único"""
        now = time.time()
        current_ids = set()
        for person in active_people:
            person_id = self.get_person_id(person)
            current_ids.add(person_id)
            if person_id not in self.active_people_ids:
                self.total_people_detected += 1  # Nova pessoa entrou
            self.active_people_ids[person_id] = now  # Atualiza último visto

        # Remove pessoas que saíram
        to_remove = [pid for pid, last_seen in self.active_people_ids.items() if now - last_seen > self.timeout_saida]
        for pid in to_remove:
            del self.active_people_ids[pid]

        # Atualiza máximo simultâneo
        if len(current_ids) > self.max_simultaneous_people:
            self.max_simultaneous_people = len(current_ids)
    ...
    def process_json_data(self, data_json):
        ...
        try:
            ...
            print(f"📡 {radar_id} | 👥 PESSOAS ATIVAS: {len(self.active_people_ids)}")
            ...
            if active_people:
                print(f"\n👥 DETECÇÕES ATUAIS ({len(self.active_people_ids)}):")
                ...
                # PREPARA DADOS PARA PLANILHA (apenas se passou 2s do último registro)
                if self.gsheets_manager:
                    if (current_time - self.last_record_time) >= 2:
                        ...
                        logger.info(f"📋 Dados adicionados: {len(self.active_people_ids)} pessoas detectadas")
            ...
            setattr(self, 'last_person_count', len(self.active_people_ids))
            print("\n" + "=" * 50)
