...
Original file is located at
    https://colab.research.google.com/drive/1re3lZtjGH8gqieqePALxh1hAJiYLLiED
"""
...
class SimpleRadarCounter:
    ...
    def __init__(self, config):
        ...
        # Novo: Sistema de reinicialização
        self.last_data_time = time.time()
        self.data_timeout = 10.0  # 10 segundos sem dados = reiniciar
    ...
    def reset_serial_connection(self):
        """Reinicia conexão serial e reseta ESP32"""
        try:
            if self.serial_connection and self.serial_connection.is_open:
                # Reseta ESP32 via DTR/RTS
                ...
                # Fecha conexão atual
                self.serial_connection.close()
                time.sleep(1)  # Aguarda fechamento completo
            ...
            if self.connect():
                ...
                logger.error(f"{self.color} ❌ Falha ao reconectar")
                return False

        except Exception as e:
            logger.error(f"{self.color} ❌ Erro ao resetar conexão: {e}")
            return False

    def force_reset(self):
        """Força reset manual da ESP32"""
        try:
            logger.info(f"{self.color} 🔄 Forçando reset manual da ESP32...")
            ...
            if self.serial_connection and self.serial_connection.is_open:
                ...
                if self.connect():
                    logger.info(f"{self.color} ✅ ESP32 resetada com sucesso!")
                    self.reconnect_attempts = 0
                    return True
                else:
                    logger.error(f"{self.color} ❌ Falha ao reconectar após reset")
                    return False
            else:
                logger.warning(f"{self.color} ⚠️ Sem conexão serial para reset")
                return False

        except Exception as e:
            logger.error(f"{self.color} ❌ Erro ao forçar reset: {e}")
            return False
        ...
        while self.is_running:
            try:
                if not self.serial_connection or not self.serial_connection.is_open:
                    ...
                    if not self.reset_serial_connection():
                ...
                if current_time - self.last_data_time > self.data_timeout:
                    logger.warning(f"{self.color} ⚠️ Sem dados seriais há {current_time - self.last_data_time:.1f}s")
                    logger.info(f"{self.color} 🔄 Tentando recuperar conexão serial (tentativa {self.reconnect_attempts + 1})...")
                    if not self.reset_serial_connection():
                        self.reconnect_attempts += 1
                        if self.reconnect_attempts >= self.max_reconnect_attempts:
                            logger.error(f"{self.color} ❌ Máximo de tentativas de reconexão atingido")
                            self.stop()
                            break
                        time.sleep(self.reconnect_delay)
                    continue
                ...
                if self.serial_connection.in_waiting:
                    try:
                        ...
                        if line:
                            ...
                            self.process_json_data(json.loads(line))
                    except json.JSONDecodeError:
                        logger.warning(f"{self.color} ⚠️ Dados inválidos recebidos")
                    except Exception as e:
                        logger.error(f"{self.color} ❌ Erro inesperado no loop: {e}")
